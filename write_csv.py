...
def get_last_line():
    with open("arbitrage_data.csv", "rb") as f:
        ...
        return str(
            int(f.readline().decode().split(",")[-1])
        )  # make sure it parses as int implicitly (or typeerror)


FIELDS_TO_GRAB = "hash,monitor_ip,sender,time_seen,payload,gas_price,gas_limit,amount,peer_name,account_nonce,id"

conn = psycopg2.connect(
    "postgres://readonly:NZOTyC4cttjvNdAKY@arbitrage3.ck0rrdngnqmh.us-west-2.rds.amazonaws.com/arbitrage?sslmode=verify-full"
)
cur = conn.cursor()
print(time.time())
...
cur.execute(
    "SELECT "
    + FIELDS_TO_GRAB
    + " FROM arbitrage WHERE id > "
    + grab_from
    + " AND id < "
    + str(int(grab_from) + 1000000)
    + ";"
)
print(time.time())
