...
logsdict = csv.DictReader(
    open("data/all_logs_bigquery.csv"),
    delimiter=",",
    quotechar='"',
...
def get_rate_label(token1, amount1, token2, amount2):
    if (token1 >= token2 and token1 != "ETH" and token1 != "WETH") or (
        token2 == "ETH" or token2 == "WETH"
    ):  # arbitrary ordering/ tiebreak
        try:
