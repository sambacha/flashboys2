...
my_provider = Web3.HTTPProvider(
    "https://mainnet.infura.io/v3/c534d76d934f40498f6d6113a46c6ab3"
)
w3 = Web3(my_provider)
...
def get_uniswap_token(address):
    ...
    erc20 = w3.eth.contract(
        address=Web3.toChecksumAddress("0x" + address), abi=UNISWAP_ABI
    )
    try:
...
def get_decimals_for(address):
    if (
        int(address, 16) == 0
        or int(address, 16) == 1364068194842176056990105843868530818345537040110
    ):
        return 18
    erc20 = w3.eth.contract(
        address=Web3.toChecksumAddress("0x" + address), abi=ERC20_ABI
    )
    try:
...
def get_node_label_for(address):
    if (
        int(address, 16) == 0
        or int(address, 16) == 1364068194842176056990105843868530818345537040110
        or address.lower() == "c0829421c1d260bd3cb3e0f06cfe2d52db2ce315"
        or address.lower() == "c02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
        or address.lower() == "2956356cd2a2bf3202f771f50d3d14a367b48070"
    ):
        return ("ETH", "ETH")
    erc20 = w3.eth.contract(
        address=Web3.toChecksumAddress("0x" + address), abi=ERC20_ABI
...
parse_etherdelta = lambda topics, data, address: parse_etherdelta_clone(
    topics, data, address, 384
)
parse_tokenstore = lambda topics, data, address: parse_etherdelta_clone(
    topics, data, address, 448
)
...
def parse_0x_v2(topics, data, address):
    abi_data = decode_abi(
        ...
        bytes(Web3.toBytes(hexstr=data)),
    )
...
def parse_uniswap_tokenpurchase(topics, data, address):
    abi_data = decode_abi(
        ...
        Web3.toBytes(hexstr="".join([x.replace("0x", "") for x in topics])),
    )
...
def get_trade_data_from_log_item(topics, data, address):
    ...
    if address == "0x8d12a197cb00d4747a1fe03395095ce2a5cc6819":  # etherdelta
        ...
        if (
            ...
            and len(topics) == 4
        ):  # (ZRXcoin has same event; eg https://etherscan.io/tx/0x3d774851984b665b6db16d8bbf7a138520c76db923599fc8929b29edd384db7b#eventlog)
            exchange = "Uniswap"
