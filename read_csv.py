...
def print_seen_line(seen_item, prev_seen_item, display_payload):
    print(
        "%.6f" % ((seen_item["time_seen"] - prev_seen_item["time_seen"]) / 10 ** 9),
        "%.6f" % (seen_item["time_seen"] / (10 ** 9)),
        seen_item["hash"],
        seen_item["sender"],
        seen_item["account_nonce"],
        seen_item["gas_price"],
        "H:" + str(hits[seen_item["hash"]]),
        seen_item["payload"] if display_payload else "",
        sep="\t",
    )
...
def should_filter_frontier(frontier, bidder_id):
    ...
    if bid_addr in frontier:
        if (
            frontier[bid_addr] > bid_nonce + 2
        ):  # (choose magic number 2 as threshold for out-of-ordering TODO repair)
            return True
...
def get_individual_auctions(seen_list):
    ...
    auctions = (
        []
    )  # list of seen lists for output auctions.  each transaction represents a "bid"
    auction_bidders = (
        []
    )  # list of set of bidders [bidder is a tuple of (hash, nonce)] in each of the above auctions, indexed similarly
    non_auction_txs = []
    # garbage_txs = [] # transactions that were a product of syncing (behind the frontier) TODO populate

    auction_participation = (
        {}
    )  # maps bidders to maps of (auction_id : tuple(str,str) to num_bid : int)
...
for monitor_ip in seen_times:
    seen_times[monitor_ip] = sorted(
        seen_times[monitor_ip], key=lambda line: line["time_seen"]
    )
...
def normalize_auction_ids(participation, auction_list):
    ...
    for bidder in participation:
        ...
        for auction_id in bidder_auctions:
            ...
            canonical_auction = (
                canonical_auction if auction_bids < best_bids else auction_id
            )
    ...
    for auction_id in canonical_bidders:
        print(
            auction_id,
            canonical_bidders[auction_id],
            len(canonical_bidders[auction_id]),
        )
            ...
            for auction in auctions_bid_in:
                if auction == canonical_auction_id:
                    ...
                    for bid in auction_list[auction]:
                        if (
                            bid["sender"] == bidder[0]
                            and bid["account_nonce"] == bidder[1]
                        ):
                            all_bids.append(bid)
