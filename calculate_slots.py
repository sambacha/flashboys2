...
for tx in slotsdict:
    ...
    if (
        int(tx["gas_used"]) < (int(tx["gas_limit"]) * 0.6)
        and int(tx["gas_price"]) > 310000000000
        and tx["log_topics"].count("~") > 1
        and not tx["to"].lower()
        in [
...
for arbitrageur in arbitrageurs.keys():
    if arbitrageurs[arbitrageur] > 0:
        print(
            "arber",
            "https://etherscan.io/address/" + arbitrageur,
        ...
        [
            ",".join(
                [
                    str(x / (10 ** 9))
                    for x in [
                        np.percentile(slotprices[slot], 10),
                        np.percentile(slotprices[slot], 50),
                        np.percentile(slotprices[slot], 75),
                        np.percentile(slotprices[slot], 90),
                        np.percentile(slotprices[slot], 99),
                    ]
                ]
            )
