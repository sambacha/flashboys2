...
query += (
    "(SELECT "
    + FIELDS_TO_GRAB.replace("num_logs,", "")
    + ", COUNT(*) AS num_logs FROM "
)
query += "(SELECT " + FIELDS_TO_GRAB.replace("num_logs,", "")
query += " FROM (SELECT " + FIELDS_TO_GRAB.replace("num_logs,", "").replace(
    "block_number", "transactions.block_number"
).replace("transaction_index", "transactions.transaction_index")
query += """  FROM
...
with open("data/all_inclfail_arb_txs_bigquery.csv", "w") as csvfile:
    ...
    spamwriter.writerow(
        "block_number,transaction_hash,to_address,from_address,gas,gas_price,receipt_gas_used,input,transaction_index".split(
            ","
        )
    )
    for item in query_job:
