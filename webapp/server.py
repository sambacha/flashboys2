...
def parse_profit_graph(profit_graph):
    ...
    for trade_index in range(0, len(profit_graph), 2):
        ...
        profitstr += (
            str(profit_graph[trade_index][2])
            + " "
            + profit_graph[trade_index][0]
            + " to "
            + str(profit_graph[trade_index + 1][2])
            + " "
            + profit_graph[trade_index + 1][1]
        )
        profitstr += "<br>"
...
def global_stats():
    ...
    return render_template(
        "global.html",
        blockstats_wfail=blockstats_wfail,
        blockstats_success=blockstats_success,
    )
...
def revenue_graphs():
    stats = query_db(
        """SELECT block_number,SUM(CAST(eth_profit as INTEGER)) AS total_profit,SUM(CAST(gas as INTEGER)) AS total_gas_bid,SUM(CAST(receipt_gas_used as INTEGER)) AS total_gas_used FROM mergedprofitabletxs WHERE all_positive="True" GROUP BY block_number ORDER BY mergedprofitabletxs.rowid DESC;"""
    )
...
@app.route(
    "/revenue/",
    defaults={"page": 1, "limit": DEFAULT_TXS_LIMIT, "start": None, "end": None},
)
@app.route(
    "/revenue/page/<int:page>",
    defaults={"limit": DEFAULT_TXS_LIMIT, "start": None, "end": None},
)
@app.route(
    "/revenue/page/<int:page>/limit/<int:limit>", defaults={"start": None, "end": None}
)
@app.route(
    "/revenue/range/<int:end>/<int:start>",
    defaults={"page": -1, "limit": DEFAULT_TXS_LIMIT},
)
def revenue(page, limit, start, end):
    ...
    txs = query_db(
        'SELECT * FROM mergedprofitabletxs WHERE rowid > ? AND rowid <= ? AND drawn="True" ORDER BY rowid DESC',
        (end_id, start_id),
    )
    profits = []
    for tx in txs:
        ...
        tx["unknown"] = (
            tx["unknown"] == "True"
        )  # todo remove this field, replace w profit graph is none
    ...
    return render_template(
        ...
        lastpage=lastpage,
        basepath="/revenue",
        graph_parser=parse_profit_graph,
        profit_parser=parse_profits,
    )
...
def strategies():
    return render_template(
        "strategies.html",
        pairwise_data=json.loads(open("../data/pairwise_self.csv").read()),
    )


@app.route(
    "/",
    defaults={"page": 1, "limit": DEFAULT_AUCTIONS_LIMIT, "start": None, "end": None},
)
@app.route(
    "/page/<int:page>",
    defaults={"limit": DEFAULT_AUCTIONS_LIMIT, "start": None, "end": None},
)
@app.route("/page/<int:page>/limit/<int:limit>", defaults={"start": None, "end": None})
@app.route(
    "/range/<int:end>/<int:start>",
    defaults={"page": -1, "limit": DEFAULT_AUCTIONS_LIMIT},
)
@app.route(
    "/auction/<int:start>",
    defaults={"page": -1, "limit": DEFAULT_AUCTIONS_LIMIT, "end": None},
)
@cached()
def hello_world(page, limit, start, end):
    # build and process auctions
    auctions = {}
    max_auction = query_db(
        "SELECT MAX(CAST(auction_id as decimal)) from auctions", one=True
    )[0]
    (start_id, end_id) = get_range(page, limit, start, end, max_auction)
    bidsdict = query_db(
        "SELECT * FROM auctions LEFT JOIN success ON success.transaction_hash=auctions.hash WHERE auction_id >= ? AND auction_id <= ?",
        (end_id, start_id),
    )
    for bid in bidsdict:
    ...
    return render_template(
        ...
        page=page,
        lastpage=int(max_auction / limit) + 1,
        hide_global_graphs=hide_global_graphs,
        deanon=("frontrun.me" in request.url),
    )
...
if __name__ == "__main__":
    if sys.argv[1] == "--dev":
        ...
        app.run(
            host="frontrun.me", debug=False, port=80, threaded=True
        )  # using dev servers in production booooo
