...
tradeAPI = (
    "0x"
    + keccak_256(b"Trade(address,uint256,address,uint256,address,address)").hexdigest()
)
...
for idx in range(max(startBlock, number), endBlock + 1, ratio):
    ...
    for txHash in transactions:
        ...
        for log in receipt["logs"]:
            if "topics" in log and len(log["topics"]):
                if log["topics"][0].hex() == tradeAPI:
                    token_pair_list.append(
                        (
                            log["data"][24 + 2 : 64 + 2],
                            log["data"][24 + 128 + 2 : 64 + 128 + 2],
                        )
                    )
        ...
        if (
            num == 2
            and token_pair_list[0][0] == token_pair_list[1][1]
            and token_pair_list[1][0] == token_pair_list[0][1]
        ):
            tag = "Arbitrage"
        elif num == 1:
            tag = "Trade"
        ...
        if tag is not None:
            result = "{} {} {} {} {} {}\n".format(
                idx, txHash.hex(), tag, tx["from"], tx["to"], tx["input"]
            )
            print(result)
